...
function updateLocationDropdowns() {
    ...
    markers.concat(userMarkers).forEach(marker => {
        ...
        storeGroups[storeType].push({ name: locationName, lat: marker.position.lat, lng: marker.position.lng, address: marker.location.address});
    });
    ...
    for (const store in storeGroups) {
        ...
        storeGroups[store].forEach(location => {
            ...
            listItem.innerHTML = `<strong>${location.name}</strong> <br> Address: ${location.address}`;
